    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(ui->action_11, &QAction::triggered, [=](){if (curC == -2) on_pushButton_clicked(); curC = 7; on_pushButton_clicked();});
    ...
    light->setQuadraticAttenuation(0.45f);
    lightEntity->addComponent(light);
    Qt3DCore::QTransform *lightTransform1 = new Qt3DCore::QTransform(lightEntity);
    lightTransform1->setTranslation(QVector3D(-5.0, 15.0, 0.0));
    lightEntity->addComponent(lightTransform1);
    Qt3DCore::QTransform *lightTransform2 = new Qt3DCore::QTransform(lightEntity);
    lightTransform2->setTranslation(QVector3D(5.0, 15.0, 0.0));
...
void MainWindow::on_pushButton_clicked()
{
    ...
    else if (curC != -2)
    {
        ...
        ui->pushButton_2->setText("Справка");
...
void MainWindow::on_startBut_clicked()
{
    ...
    if (!timer->isActive())
    {
        ...
        lTime->setText("Время: 00:00.00");
        m->lock(true);
        ui->startBut->setText("Завершить эксперимент");
        timer->start();
    }
    else
    {
        timer->stop();
        m->lock(false);
        ui->startBut->setText("Начать эксперимент!");
    }
}
